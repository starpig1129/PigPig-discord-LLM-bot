                    ...
                    if msg.attachments:
                        for att in msg.attachments:
                            if att.id in self._attachment_cache:
                                ...
                                # Cache miss: prepare to process
                                attachment_tasks.append((att.id, process_attachment(att)))
                                task_mapping.append(msg.id)

            if attachment_tasks:
                import asyncio
                # Extract just the coroutines for gather
                coros = [task[1] for task in attachment_tasks]
                results = await asyncio.gather(*coros, return_exceptions=True)

                for task_info, msg_id, res in zip(attachment_tasks, task_mapping, results):
                    att_id = task_info[0]
